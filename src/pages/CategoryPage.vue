<template>
    ...
    <div class="container mx-auto lg:px-20 px-4">
        <div class="text-center mb-16">
            ...
            <p v-intersection-observer="updateTextAnimation" class="translate-x-0 translate-y-0 opacity-100 transition-all duration-500 text-animation text-sm text-[#282828] px-4">
                {{ category.description }}
            </p>

            <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 mt-12 gap-8" v-if="clinics.totalItems">
                <div
                    v-intersection-observer="toggleClass"
                    v-for="clinic of clinics.models"
                    :key="clinic.id"
            ...
            <div class="text-center text-black text-lg font-semibold p-4" v-else>
                {{ $t('listIsEmpty') }}
            </div>
        </div>
    </div>
...
onMounted(() => {
    store.dispatch('fetchCategory', route.params.id)
    store.dispatch('fetchClinics', route.params.id)
})
</script>
